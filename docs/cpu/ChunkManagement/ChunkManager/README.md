...
`ChunkManager`는 Voxen 월드의 모든 청크를 중앙에서 지휘하는 관리자다. Chunk가 자기 자신의 CPU 데이터만 소유하는 순수한 데이터 집합이라면([ChunkStructure](../ChunkStructure/README.md) 참조), ChunkManager는 그 위에서 다음 책임을 진다.
...
일반적으로 Manager 관련 시스템 로직은 Signleton으로 작성된다.
사용이 간단하고 메모리가 하나만 존재한다는 장점과 유지보수가 어려워지는 단점도 존재하지만, 여러 곳에서 사용하게 되기에 싱글톤을 채택하였다.

메모리 인스턴스 변수를 함수 내부에 static 변수로 두어 C++11 멀티쓰레드에도 안정성을 갖게 구성하였다.
...
ChunkManager가 매니저다운 역할을 할 수 있는 핵심 근거는 **모든 재사용 대상 리소스를 시작 시점에 한꺼번에 할당해 둔다**는 것이다.
매 프레임 루프에서 `new`/`delete`, `CreateBuffer` 같은 비용 큰 호출을 최대한 피한다.

풀 패턴을 클래스 내부에 두고 생성자/소멸자로 RAII 맞춰 사용할 수 있으나, 단순히 직접 빌려오고 반납하는 식으로 작성을 하였다.
...
풀 대여·반환은 직접 호출한다.: `GetChunkLoadMemoryFromPool()`로 대여, 작업 종료 후 `ReleaseChunkLoadMemoryToPool()`로 반환하며, 반환 직전 `Clear()`로 상태를 초기화한다.

- 풀 대여반환을 조금 더 안전하게 사용하려면 생성자 소멸자에 두고 RAII를 지키며 사용할 수 있을 것이다.
...
이 셋을 모두 풀링해 두면, **로드 중 발생하는 유일한 할당은 CPU 메시 vector의 push_back**뿐이다. 그것도 vector의 `capacity`는 유지된 채로 재사용되므로 실질적 비용은 초회 로드 주로 생긴다.

## 4. GPU 버퍼가 왜 Chunk가 아닌 ChunkManager에 있는가

Chunk는 CPU 메시 데이터만, GPU 버퍼는 ChunkManager가 소유한다.
...
- 청크마다 Instance 렌더링을 하기에는 쓸데없는 DrawCall이 많다고 판단했고, Instance 렌더의 효과를 살려 한번에 모아 처리하기를 원했기 때문이다.
...
즉 **Instance GPU 버퍼는 청크의 소유물이 아니라 렌더 시스템의 공유 자원**이다. 청크 단위로 그리지 않으니 청크가 들고 있을 수도 없다. 자연히 ChunkManager로 올라왔다.

- Block 정보는 Chunk가 CPU&GPU 데이터를 가지고, Instance 정보는 Chunk가 CPU만, ChunkManager가 GPU 데이터를 가지게 됐었다.

여기서 **이원 구조 문제**가 발생했다. 인스턴스 버퍼는 Manager, 블록 버퍼는 Chunk에 있게 되면서:

- GPU 리소스 리사이즈·업로드 코드가 두 곳으로 갈라짐
- DX11 디바이스/컨텍스트 접근 경로가 두 위치에서 필요
- 스레드 경계(워커는 CPU 데이터, 메인은 GPU 업로드)가 모호함
...
Instance Info 버퍼는 위와 성격이 조금 다르다. 매 프레임 렌더 대상 청크가 바뀌면서 Shape별 Instance 개수가 다양하다.
...
ChunkManager의 심장부는 `Update(dt, camera, light)`다.
...
│
├─ [2] LoadChunks                       (async)
│      → m_waitLoadChunkPosMap 관리
│      → 카메라 거리순 정렬 후 std::async(Chunk::Initialize) 시작
...
│
├─ [5] PatchChunks                      (async)
│      → 카메라 거리순 정렬 후 std::async(Chunk::Patch) 시작
...
`LoadChunks`는 **디스패치**만, `SyncLoadedChunks`는 **동기화**만 한다.

- **코드 분리**: Load는 단순히 빈 쓰레드 자리가 있으면 실행만, Sync는 로드가 완료된 청크에 대해서 여러가지 검사 등 분리할 필요가 존재했다.

Patch 쪽도 같은 이유로 `PatchChunks` / `SyncPatchedChunks`로 나뉜다.

### 6.2 UpdateLoadUnLoadChunkList 트리거 실행

`UpdateLoadUnLoadChunkList`는 매 프레임 호출되지 않는다. `m_isOnChunkUpdateDirtyFlag`가 켜진 프레임에서만 실행된다.

이 플래그는 카메라가 청크 경계를 넘을 때 `OnChunkUpdateDirtyFlag()`로 켜진다.

청크 좌표가 그대로면 로드/언로드 대상이 바뀌지 않으므로 재계산이 무의미하다.
...
각 필요에 따라 주체(`App`)가 Render 함수를 호출하면 내부적으로 관련된 렌더 함수를 더 호출하게 된다.

다양한 상태에 맞춰 적절히 PSO를 구성했다.
...
- CPU-GPU 관리는 어떻게 해야할지
- 매 프레임 Update는 어떻게 해야지 안전하고 카메라 속도와 관련없이 어떻게 모든 청크를 로드할 수 있을지
- 멀티쓰레드에서 레이스컨디션을 아예 회피하기 위해 메모리를 어떻게 분리할지
- 메모리를 분리했지만 이 메모리를 로드 시 마다 재할당으로 느려지니까 메모리를 로드 메모리를 할당해야하는지

Chunk를 나는 단순히 Map으로 관리했지만 Octree로 관리할 수 있다고 한다.

쓰레드풀을 사용하지 않고 쓰레드를 사용 중이며, 메모리 풀 관리에 RAII를 직접 내 손으로 하고 있는 미흡한점도 많다.

Load/Unload/Patch 흐름이 가장 핵심적이였고, 추가적인 버그나 예외는 발견되지 않아서 만족한다.
