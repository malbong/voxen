...
사용이 간단하고 메모리가 하나만 존재한다는 장점과 유지보수가 어려워지는 단점도 존재하나, 여러 군데에서 사용하는 방식에 따라 싱글톤을 채택하였다.

instance 변수를 함수 내부에 static 변수로 두어 C++11 멀티쓰레드에도 안정성을 갖게 구성하였다.
...
풀 패턴을 좀 더 RAII 맞춰 사용할 수 있으나, 단순히 직접 빌려오고 반납하는 식으로 작성을 하였다
