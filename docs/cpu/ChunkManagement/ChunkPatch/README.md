...
static void PatchData::GenerateEdgePatchEntry(int x, int y, int z, Vector3 chunkPosition,
    BLOCK_TYPE blockType, int baseSize, std::pair<PosInt3, PatchData>* outEntry, int& outCount)
{
    int xEdgeDir = (x == 0) ? -1 : ((x == baseSize - 1) ? 1 : 0);
    if (xEdgeDir) {
        int newX = (xEdgeDir == -1) ? baseSize : -1;    // 32 또는 -1
...
이 함수는 `PatchData`의 static 메서드로 두었다. 원본 청크의 정보만 있으면 이웃 청크에 넣을 `PatchData`를 만들 수 있고, `ChunkManager`의 어떤 인스턴스 상태에도 의존하지 않으므로 `PatchData` 자체가 자기 자신을 만드는 로직을 소유하는 것이 자연스럽다. 나무 초기화 경로(`Chunk::Initialize`)와 플레이어 피킹 경로(`ChunkManager::PropagatePatchByEdgeBlock`)가 같은 로직을 공유한다.
    ...
    Vector3 localPosition, Vector3 chunkOffsetPos, BLOCK_TYPE blockType)
{
    ...
    PatchData::GenerateEdgePatchEntry(localPosition, chunkOffsetPos, blockType,
        Chunk::CHUNK_SIZE, outEdgePatchEntry, count);
...
PatchData p(localPos, replaced, Instance(), CHUNK_SIZE, false);
...
- **face 오프셋으로 인한 크로스 청크 블록 추가**는 잘 동작하지만, "조준 청크"와 "패치 청크"가 다를 수 있다는 사실이 원거리 코드 리뷰 시 놓치기 쉽다. `PatchData` 생성자가 좌표 랩(wrap)을 처리할 수 있는 파라미터를 이미 갖고 있으니, 이를 활용해 조준 청크 기준으로 통일하는 리팩토링도 고려할 만하다.
