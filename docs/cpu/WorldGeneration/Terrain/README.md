...
원래는 단순히 heightMap를 구성하기 위해 Perlin 노이즈를 섞기도 하고 진동수 옥타브를 바꿔가며 height를 최종 결정했다.
...
### 4.1 노이즈 c, e, pv를 섞어 적절히 높이 지형은 구성했지만, Biome을 고려하지 않아 사막이 산 꼭대기에 존재하는 문제

높이에 따라 Biome을 구분할까도 생각했지만 적절하지 않았다.
온도,습도에 높이를 추가하면 3D인 상황에 바이옴 분리하기 쉽지 않았다.

**해결**: Biome이 높이 맵에 영향을 받는게 아닌, 높이가 Biome에 영향을 주는 방향으로 진행

### 4.2 최근접 biome 만 잡으면 경계가 불연속
