...
이미 완성된 화면에 BlendState를 활용하여 Blending한다.

일반적으로 Fog를 후처리 과정에서 완성된 화면(basicMSRTV)와 깊이 버퍼를 입력(SRV)로 받아 사용하지만,
해당 프로젝트는 Forward Rendering 중간에 Fog가 들어가기 때문에 Self Reference를 피하려고 `CopyResource`를 사용해야만 한다.
그래서 `CopyResource` 비용없이 Forward Rendering의 장점을 살려 단순히 FogFactor를 Alpha값으로 사용하여 Blending한다.

```
Deferred Shading 결과 (basicMSRTV)  ─┐
...
FogFilterPS.hlsl은 풀스크린 쿼드의 Pixel Shader로, 다음 입력을 받는다.

| 리소스              | 레지스터 | 타입                 | 내용                                |
| ------------------- | -------- | -------------------- | ----------------------------------- |
| `depthTex`          | t0       | `Texture2DMS<float>` | Depth Buffer (MSAA)                 |
...
- Forward Rendering Pass 중간에 Fog가 섞여 MSAA FullScreen 후처리라는게 마음에 걸린다.
  - PostEffect Pass에 넣어볼까도 생각하고 Deferred Rendering Pass에 넣어볼까도 생각했지만, Water가 존재하는한 어려웠다.
