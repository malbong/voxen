...
- 단순히 정적 큐브맵을 사용하기보다 더 효과적인 배경을 구현할 수 있었음
  - 동적인 환경에 따른 간접광이나 직접광의 추가적인 구현 복잡도는 다른 곳에서 올라가는 문제는 존재했음
- 시간에 따른 하늘색과 보간의 방식의 경우의 수, 간접조명엔 어떤 영향을 줄건지, `posDir`만을 가지고 텍스쳐를 어떻게 만들지에 관련된 고민이 많았지만 하나씩 해결할 수 있었음
