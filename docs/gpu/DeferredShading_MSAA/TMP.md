...
1. **마킹 값 기반 1차 필터** — `normalEdge.w` 값으로 각 샘플을 분류하고 두 가지 Edge 조건을 검사한다:
   ...
   - 단순히 Coverage값만 이용해 Edge가 아닌 경우 discard해버리면, SemiAlpha가 Clip된 샘플의 경우에도 Edge 판단을 하지 못하는 경우가 있다. 그래서 SemiAlpha 마킹도 필요하다.
   - 아래는 SemiAlpha 메쉬 검사를 하지 않는 경우와 하는 경우다.
...
`semiAlphaCount == SAMPLE_COUNT`인 경우(모든 4샘플이 SemiAlpha)는 에지로 처리하지 않는다.
SemiAlpha 패스는 오브젝트 렌더 순서상 불투명 오브젝트 이후에 실행되므로, 모든 샘플이 SemiAlpha라면 해당 픽셀은 내부 픽셀로 확정된다.
